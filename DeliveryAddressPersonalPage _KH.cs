...
{
    public class DeliveryAddressPersonalPage_KH : Page
    {
        ...
        [FindsBy(How = How.XPath, Using = "//*[@id='new-saved-address-']")]
        ...
        [FindsBy(How = How.XPath, Using = "//*[@id='City']")]
        public IWebElement StreetDl { get; set; }

        [FindsBy(How = How.XPath, Using = "//*[@id='Street2']")]
        public IWebElement StateCode { get; set; }
        ...
        public IWebElement ErrorMsgCounty { get; set; }
        ...
        {
            StreetDl.SelectDropDownOption(option);
        }

        public void SelectStateCode(string option)
        {
            StateCode.SelectDropDownOption(option);
        }
        ...
        public Framework.Address AddAddress(Framework.Address add, bool IsCustomer = false, bool iscancelled = false)
        {
            ...
            PageExtension.WaitForTimeSpan(2000);
            SelectState(add.State);
            PageExtension.WaitForTimeSpan(2000);
            SelectCity(add.City);
            PageExtension.WaitForTimeSpan(2000);
            SelectStreet(add.Street2);
            PageExtension.WaitForTimeSpan(2000);
            SelectStateCode(add.StreetNumber);
            PageExtension.WaitForTimeSpan(2000);
        ...
        public void SearchAndEdit(Framework.Address add, bool IsCustomer = false)
        {
            ...
            if (count1 != null && count1.Count > 0)
            {
                foreach (var item in count1)
                {
                    ...
                    add.ClearAddress();
                    add.FillAddressforSAMCAM("FN", "LN", "StrAddress", "សង្កាត់កោះអន្លង់ចិន", "ក្រុងតាខ្មៅ", "ខេត្តកណ្ដាល", "567894", "0987654321", "កណ្ដាលកោះ", "District", "Area"); PageExtension.WaitForTimeSpan(2000);
    ...
    public class DeliveryAddressPersonalPageTestKH : DeliveryAddressPersonalPage_KH { }
}